...
export default function Navigation({ variant = 'dark' }: NavigationProps) {
  ...
  return (
    <>
      {/* Background Blur Overlay - 모바일 메뉴 열릴 때만 표시 */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div
            initial={{ opacity: 0 }}
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className={`md:hidden border-t ${isDark ? 'bg-black/95 backdrop-blur-md border-white/10' : 'bg-white/95 backdrop-blur-md border-gray-200'}`}
            >
